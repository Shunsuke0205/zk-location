...
/// Placeholder aggregator AIR: checks parent = left + right (lane-wise) and carries C through.
...
impl<F: Field> BaseAir<F> for OutsideAggAir { fn width(&self) -> usize { 4 /*left*/ + 4 /*right*/ + 4 /*parent*/ + 4 /*C lanes*/ } }
...
where AB::F: Field + PrimeCharacteristicRing {
    ...
    let _ccols = (0..4).map(|i| row.get(0, base + 12 + i).unwrap()).collect::<Vec<_>>();
        // Bind PVs: [left(4), right(4), parent(4), C(4)]
        if pvs.len() >= 16 { for i in 0..16 { builder.assert_eq(row.get(0, i).unwrap(), pvs[i]); } }
        for i in 0..4 { builder.assert_eq(parent[i].clone(), left[i].clone() + right[i].clone()); }
        // C is unconstrained beyond binding; in real verifier, we would check C_left==C_right==C.
    }
}

#[allow(dead_code)]
pub fn build_trace_outside_agg(left: [u32;4], right: [u32;4], c: [u32;4]) -> RowMajorMatrix<BabyBear> {
    let mut row = vec![BabyBear::ZERO; 16];
    for i in 0..4 { row[i] = BabyBear::new(left[i]); }
    for i in 0..4 { row[4+i] = BabyBear::new(right[i]); }
    for i in 0..4 { row[8+i] = BabyBear::new(left[i].wrapping_add(right[i])); }
...
pub fn prove_outside_agg(left: [u32;4], right: [u32;4], c: [u32;4]) -> p3_uni_stark::Proof<MyConfig> {
    ...
    let parent = [left[0].wrapping_add(right[0]), left[1].wrapping_add(right[1]), left[2].wrapping_add(right[2]), left[3].wrapping_add(right[3])];
